...
La autenticidad de un item en la cadena de bloques no depende de una autoridad central, es descentralizado, en nuestro caso, la autenticidad de una carta en TraDeck. Cada activo, tokenizado como un token no fungible (NFT) tiene un identificador cuya validez se verifica mediante la continuidad del libro contable, el ledger.

Si definimos un bloque como $B_n$, su validez está condicionada por la inclusión del identificador único del bloque anterior, creando una estructura de punteros \textit{hash}. Formalmente, un bloque se define como:
...
Mediante esta arquitectura, logramos un equilibrio: la eficiencia económica de no saturar el ledger de um, manteniendo la robustez transaccional e inmutabilidad estricta exigida en un sistema de bases de datos de alta seguridad.
