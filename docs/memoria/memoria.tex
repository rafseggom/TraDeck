...
\setTitle{TraDeck: Plataforma descentralizada para intercambio de cartas coleccionables tokenizadas}
\setAuthor{Javier Luque Ruiz\\Rafael Segura Gómez} % Si hay más de un autor, separarlos con \\
\setDegree{Grado en Ingeniería Informática - Ingeniería del Software} % Cambiar si es necesario
\setSupervisor{María Teresa Gómez López\\Octavio Martín Díaz\\Antonia María Reina Quintero} % Si hay más de un tutor, separarlos con \\
\setDepartment{Lenguajes y Sistemas Informáticos}
\setMonth{26 de abril} % Dejar sólo el mes de la convocatoria en que se presenta el trabajo
\setYear{2026} % Por ejemplo, 2022/23
...
\begin{document}
    ...
    % Portada y secciones no n umeradas
    ...
    \input{sections/02_Arquitectura_de_Datos}
    \input{sections/03_Logica_Transaccional}
    \input{sections/04_Implementacion}
    \input{sections/05_Manuales}
    \input{sections/06_Conclusiones}
    \input{sections/07_IA}

    % Bibliografía
    \bibliographystyle{unsrtnat}
    \bibliography{bibliografia.bib}
    
    % Nuevo capítulo de material de consulta
    \input{sections/08_Material_Consulta}
